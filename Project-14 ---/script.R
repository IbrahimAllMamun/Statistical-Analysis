...
#   T2  Multivariable logistic regression: NEWS(d5) -> mortality
#   T3  Multivariable logistic regression: NLPR(d5) -> mortality
#   T4  ROC: cut-off / specificity / sensitivity / AUC / 95% CI / p
#       for NEWS d1,d3,d5 and NLPR d1,d3,d5 + combined model
#
# NB: the paper's marker of interest in T2/T4 is NLR. At the client's
# request NEWS takes its place there, so NLR now appears only as a
# descriptive row in Table 1e.
#
...
#   * APACHE II was not collected and is dropped from the Table 2 / 3
#     adjustment set, which is sex, age group and SOFA. NEWS cannot serve
#     as a covariate there either - it separates the outcome groups (see
#     below) and makes every coefficient unidentifiable - but it is a
#     valid EXPOSURE in Table 2, where Firth handles the separation.
...
roc_row <- function(score, outcome_bin, label) {
  ...
  # PPV / NPV are read off the same Youden-optimal cut-off, so they carry the
  # observed mortality of this cohort as the prior - they are not transportable
  # to a setting with a different death rate.
  ...
  tibble(
    ...
    PPV             = sprintf("%.3f", co$ppv),
    NPV             = sprintf("%.3f", co$npv),
    AUC             = sprintf("%.3f", a),
    `95% CI`        = ci_txt,
...
table4_df <- bind_rows(
  roc_row(analysis$news_day1, analysis$death, "NEWS (d1)"),
  roc_row(analysis$news_day3, analysis$death, "NEWS (d3)"),
  roc_row(analysis$news_day5, analysis$death, "NEWS (d5)"),
...
# nine columns now, so fit the table to the page width rather than to content
...
roc_png(c("news_day1", "news_day3", "news_day5"),
        c("NEWS day 1", "NEWS day 3", "NEWS day 5"),
        "Graph/Fig1_ROC_NEWS.png")
...
foot23 <- paste0(
  ...
  "used because both NEWS (d5) and NLPR (d5) separate the two outcome groups completely and ",
  "ordinary maximum likelihood does not converge. Adjusted for sex, age group and SOFA, with age ",
  "<40 years as the reference group. The APACHE II term of the reference paper has no counterpart ",
  "here and is omitted: APACHE II was not recorded in this study. ",
  "A confidence bound shown as \">1000\" or \"<0.001\" is not estimable: the ",
...
foot4 <- paste0(
  "Cut-off is the Youden-optimal point. NEWS is an integer score, so its cut-off falls between two ",
  "whole numbers. The AUC confidence interval and the test of AUC = 0.5 use ",
  ...
  "PPV and NPV are evaluated at the same cut-off and therefore embed this cohort's ",
  sprintf("in-hospital mortality of %.1f%% as the prior; they do not transfer to a population ",
          100 * mean(analysis$death, na.rm = TRUE)),
  "with a different death rate, whereas sensitivity, specificity and the AUC do. ",
  "AUC, area under the ROC curve; CI, confidence interval; PPV, positive predictive value; ",
...
doc <- doc %>%
  body_add("Table 2. Multivariable logistic regression exploring the association of NEWS (d5) with in-hospital mortality",
           style = "heading 1") %>%
  ...
  body_add("Table 4. Predictive value of NEWS and NLPR at different time points for in-hospital mortality of sepsis patients",
           style = "heading 1") %>%
...
# ----------------------------------------------------------
note("")
note("--- Table 2: NEWS(d5) model (Firth) ---")
...
safe_write(writeLines(LOG, OUT_LOG), OUT_LOG)
message("Saved -> Graph/Fig1_ROC_NEWS.png")
